...
bool Connection::open(const char* address)
{
...
// Send data
int Connection::sendData(char* data)
{
	return send(s, data, strlen(data), NULL);
}

// Send data
int Connection::sendData(char* buf, int len)
{
	return send(s, buf, len, NULL);
}
